...
path = '/media/samantha/My Passport/these_sillage_plots/mediciones_wake_2/'
...
for ii in range(len(vs)):
   ...
   #  idx_vertical_f = 1000
    ...
    # plt.plot(Rmean_mur, label=mur)
    # plt.plot(Rmean_gra, label=gra)
...
plt.figure()
plt.plot(vs, Rs_mur, '.-', label='mur')
plt.plot(vs, Rs_gra, '.-', label='gra')
plt.errorbar(vs, Rs_mur, yerr=eRs_mur,  label='mur')
plt.errorbar(vs, Rs_gra, yerr=eRs_gra,  label='gra')
plt.legend()
plt.grid()
plt.show()
