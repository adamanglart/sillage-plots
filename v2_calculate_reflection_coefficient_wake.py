...
idxs1 = np.arange(100,200)
kx1   = np.zeros_like(idxs1, dtype=float)
kx2   = np.zeros_like(idxs1, dtype=float)
absR  = np.zeros_like(idxs1, dtype=float)
kk = 0
for  idx1 in idxs1:
    #idx1 = 200
    idx2 = idx1 + 50
    ...
    kx1[kk]  = kx_calculated_1
    kx2[kk]  = kx_calculated_2
    absR[kk] = np.abs(R_calculated)
    kk = kk + 1
