...
path = '/media/samantha/My Passport/soutenance_plots/wake/'
...
height_mur = np.load(path + ' mur_'+str(vs[ii])+'.npy')
height_gra = np.load(path + ' gra_'+str(vs[ii])+'.npy')
...
first_idx = 430
hmur_extended = hmur_extended[:,first_idx:]
hgra_extended = hgra_extended[:,first_idx:]
mask_mur_extended = mask_mur_extended[:,first_idx:]
mask_gra_extended = mask_gra_extended[:,first_idx:]
...
A = 0.08
dx = 0.08
ax[0].arrow(0.32-dx, -0.08, A*np.cos(theta), A*np.sin(theta), width = 0.0004, head_width=0.006, facecolor='k', edgecolor='k')
ax[0].arrow(0.12-dx, -0.08 + A*np.sin(theta), A*np.cos(theta), -A*np.sin(theta), width = 0.0004, head_width=0.006, facecolor='k', edgecolor='k')
ax[0].text(0.14-dx, -0.085, '$R$')
ax[0].text(0.36-dx, -0.085, '$\mathbf{k}$')

ax[1].arrow(0.32-dx, -0.08, A*np.cos(theta), A*np.sin(theta), width = 0.0004, head_width=0.006, facecolor='k', edgecolor='k')
ax[1].arrow(0.12-dx, -0.08 + A*np.sin(theta), A*np.cos(theta), -A*np.sin(theta), width = 0.0004, head_width=0.006, facecolor='k', edgecolor='k')
ax[1].text(0.14-dx, -0.085, '$R$')
ax[1].text(0.36-dx, -0.085, '$\mathbf{k}$')



for i in range(hmur_extended.shape[1]//periodicity_grating):
...
cbar = fig.colorbar(im0, ax=ax.ravel().tolist(), fraction=0.046, pad=0.04, shrink = 0.895)
cbar.ax.set_ylabel('h [mm]', rotation=270, labelpad=20)

plt.show()
plt.savefig('/home/samantha/Dropbox/PhD/these/figures/wake/new_sillage_experimental_' + '%d' % vs[ii]  +'.png')
plt.close()
